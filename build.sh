...
# Keep tools/hookproj.py self-contained: regenerate its embedded copy of the SDK JAR.
python3 - <<'PY'
from pathlib import Path
import base64, hashlib, textwrap
jar = Path('out/ij2art-hook-api.jar').read_bytes()
digest = hashlib.sha256(jar).hexdigest()
b64 = base64.b64encode(jar).decode()
lines = '\n'.join(f"    '{chunk}'" for chunk in textwrap.wrap(b64, 76))
block = ("# BEGIN_EMBEDDED_HOOK_API_JAR (generated by build.sh; do not edit)\n"
         f"_EMBEDDED_HOOK_API_JAR_SHA256 = '{digest}'\n"
         "_EMBEDDED_HOOK_API_JAR_B64 = (\n" + lines + "\n)\n"
         "# END_EMBEDDED_HOOK_API_JAR")
script = Path('tools/hookproj.py')
text = script.read_text()
begin = text.index('# BEGIN_EMBEDDED_HOOK_API_JAR')
end = text.index('# END_EMBEDDED_HOOK_API_JAR') + len('# END_EMBEDDED_HOOK_API_JAR')
script.write_text(text[:begin] + block + text[end:])
PY
